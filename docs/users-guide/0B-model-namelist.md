...
|---|---|
| Units | - |
| Description | Large-eddy-simulation dissipation model |
...
|---|---|
| Units | - |
| Description | Large-eddy-simulation surface-flux option |
| Possible Values | `'none'`, `'specified'`, or `'varying'` *(default: none)* |
...
|---|---|
| Units | K m s^-1 |
| Description | Specified surface kinematic heat flux, w'theta' |
| Possible Values | Real values *(default: 0.0)* |
...
|---|---|
| Units | - |
| Description | 10 m surface drag coefficient |
| Possible Values | Real values *(default: 0.0)* |
...
|---|---|
| Units | - |
| Description | Value of the vertically implicit off-centering parameter below its transition zone |
| Possible Values | Real values between 0 and 1 *(default: 0.1)* |
...
|---|---|
| Units | - |
| Description | Value of the vertically implicit off-centering parameter above its transition zone |
| Possible Values | Real values between 0 and 1 *(default: 0.5)* |
...
|---|---|
| Units | m |
| Description | Height MSL at the bottom of the epssm transition zone |
| Possible Values | Positive real values *(default: 30000.)* |
...
|---|---|
| Units | m |
| Description | Height MSL at the top of the epssm transition zone |
| Possible Values | Positive real values *(default: 50000.)* |
...
|---|---|
| Units | - |
| Description | Whether to use GPU-aware MPI for halo exchanges; implemented only with `config_halo_exch_method = 'mpas_halo'` *(hidden by default)* |
| Possible Values | .true. or .false. *(default: false)* |
...
|---|---|
| Units | m |
| Description | Height above which microphysics tendencies are excluded *(hidden by default)* |
| Possible Values | Positive real values *(default: 45000.)* |
...
## B.14 chemistry, photolysis, lnox

The `&chemistry`, `&photolysis`, and `&lnox` namelist records configure
...
### `config_chem_tracer_output_unit` (character)

| | |
|---|---|
| Units | - |
| Description | Global history-output unit for MICM chemical tracers. `mmr` preserves the authoritative `q<species>` dry-air mass mixing ratios and creates no companion fields. A non-MMR value adds output-only `vmr_<species>` diagnostics without changing transport, MICM coupling, input, or restart state. |
| Possible Values | `mmr`, `fraction`, `percent`, `ppmv`, `ppbv`, or `pptv` *(default: `mmr`)* |

### `config_chem_tracer_output_overrides` (character)

| | |
|---|---|
| Units | - |
| Description | Comma-separated `SPECIES=UNIT` overrides, using exact case-sensitive MICM species names and the units accepted by `config_chem_tracer_output_unit`. Overrides take precedence over the global value; `SPECIES=mmr` suppresses that species' VMR companion. |
| Possible Values | Empty or a comma-separated mapping such as `O3=ppmv,NO=ppbv,NO2=ppbv` *(default: '')* |
...
|---|---|
| Units | ppbv s^-1 |
| Description | Lightning NOx source-rate amplitude. Altitude mode scales it by `(w - w_threshold) / w_ref`; isotherm mode applies it as a constant. Zero disables either mode. |
| Possible Values | Any non-negative real *(default: 0.0)* |

### `config_lnox_gating_mode` (character)

| | |
|---|---|
| Units | - |
| Description | Lightning-NOx gating mode. `altitude` uses the height bounds and linear vertical-velocity scaling; `isotherm` uses the temperature bounds and a constant configured source rate. Any other value is fatal during initialization. |
| Possible Values | `altitude` or `isotherm` *(default: `altitude`)* |

### `config_lnox_t_min` (real)

| | |
|---|---|
| Units | K |
| Description | Lower (cold) temperature bound for isotherm-mode lightning-NOx injection |
| Possible Values | Positive real values less than `config_lnox_t_max` *(default: 233.15)* |

### `config_lnox_t_max` (real)

| | |
|---|---|
| Units | K |
| Description | Upper (warm) temperature bound for isotherm-mode lightning-NOx injection |
| Possible Values | Positive real values greater than `config_lnox_t_min` *(default: 262.15)* |
...
|---|---|
| Units | m s^-1 |
| Description | Excess-vertical-velocity scale used by the altitude formula `S = rate * max(0, w - w_threshold) / w_ref`. The current initializer requires a positive value for both modes even though the isotherm formula does not use it. |
...
|---|---|
| Units | m |
| Description | Minimum altitude for lightning NOx injection in height above MSL from `zgrid` (not AGL). |
...
|---|---|
| Units | m |
| Description | Maximum altitude for lightning NOx injection in height above MSL from `zgrid` (not AGL). |
| Possible Values | Any non-negative real *(default: 12000.0)* |

### `config_j_no2_max` (real)

| | |
|---|---|
| Units | s^-1 |
| Description | Maximum daytime NO2 photolysis rate used in the single-rate fallback `j = j_max * max(0, cos(SZA))`. A positive value enables that fallback when TUV-x is inactive and requires `PHOTO.jNO2` in the MICM mechanism; zero drives no fallback photolysis parameter. Ignored when TUV-x is active. |
| Possible Values | Any non-negative real *(default: 0.0)* |
...
|---|---|
| Units | s |
| Description | NOx sink timescale (0 = no sink). When positive, sets all MICM rate parameters whose names begin with `LOSS.` to `1 / tau`. |
| Possible Values | Any non-negative real *(default: 0.0)* |
...
|---|---|
| Units | - |
| Description | TUV-x configuration file name. When empty, a positive `config_j_no2_max` selects the single-rate `cos(SZA)` fallback; when both are empty/zero, no photolysis parameter is driven. |
| Possible Values | Any valid filename *(default: '')* |
...
|---|---|
| Units | - |
| Description | Compatibility consistency flag for an upper-column extension. Set `.true.` with `legacy_static` or `spatial_climatology`, and `.false.` with `none`. |
| Possible Values | .true. or .false. *(default: false)* |

### `config_tuvx_upper_column_mode` (character)

| | |
|---|---|
| Units | - |
| Description | Explicit upper-column selection. `none` uses only the MPAS column; `legacy_static` uses the one-dimensional CSV; `spatial_climatology` uses an exact-grid cyclic monthly NetCDF package. Inconsistent combinations are fatal. |
| Possible Values | `none`, `legacy_static`, or `spatial_climatology` *(default: `none`)* |
...
|---|---|
| Units | - |
| Description | Path to an exact-grid `chempas-prescribed-field-package-v1` monthly NetCDF package. Required only in `spatial_climatology` mode. |
| Possible Values | Any valid filename *(default: '')* |
...
### `config_chemistry_interval` (real)

| | |
|---|---|
| Units | s |
| Description | Chemistry solve interval in simulated seconds. `0.0` runs chemistry every MPAS step. Positive values accumulate elapsed dynamics timesteps and skip chemistry until the interval is reached; MIEM sampling, LNOx injection, and the MICM solve then use the accumulated chemistry timestep. |
| Possible Values | Any non-negative real *(default: 0.0)* |
...
|---|---|
| Units | - |
| Description | Allocate and run a duplicate chemistry-only MICM state. It receives the same photolysis and MIEM parameters but no subsequent MPAS advection, environmental recoupling, or operator-split LNOx, so differences are not a pure transport attribution unless those other inputs are static or absent. Doubles chemistry cost. |
| Possible Values | .true. or .false. *(default: false)* |
...
|---|---|
| Units | - |
| Description | Number of outer MICM substeps per fired chemistry interval. The production `RosenbrockStandardOrder` solver is called N times with the accumulated `dt_chem/N`; useful for stiff or fast-transient mechanisms. |
...
|---|---|
| Units | - |
| Description | Relative error tolerance for the production `RosenbrockStandardOrder` solver. Tightening it (for example, from the 1e-6 default to 1e-9) makes the adaptive controller apply stricter acceptance criteria and may increase internal work. |
| Possible Values | Any positive real *(default: 1.0e-6)* |

### `config_chem_max_failures` (integer)

| | |
|---|---|
| Units | - |
| Description | Maximum number of consecutive recoverable chemistry-step failures before the model aborts. A successful transactional chemistry step resets the counter. |
| Possible Values | Any positive integer *(default: 3)* |

## B.15 emissions

### `config_miem_file` (character)

| | |
|---|---|
| Units | - |
| Description | MUSICA mechanism-configuration file containing an emissions section. An empty path disables MIEM. Inventory cells must already use the exact global MPAS mesh and `indexToCellID` order; enabled ranks read only their selected owned cells and validate returned geometry in-model. |
| Possible Values | Empty or any valid filename *(default: '')* |

The path is resolved from the MPAS process working directory. This option is
independent of `config_micm_file`: the latter selects chemistry and supplies
the writable `EMIS.*` rate parameters. See
[MIEM_INTEGRATION.md](../chempas/musica/MIEM_INTEGRATION.md) for required
prelaunch inventory validation.

### `config_miem_net_flux_species` (character)

| | |
|---|---|
| Units | - |
| Description | Comma-separated exact MIEM species whose prescribed surface exchange may be positive (upward source) or negative (downward uptake). Every omitted species retains the source-only nonnegative contract. Names must match both the MIEM species and writable `EMIS.*` parameter exactly. |
| Possible Values | Empty or comma-separated exact MIEM species names *(default: '')* |

This opt-in is intended for signed products such as posterior CH4 surface
exchange. It does not infer sink semantics from a negative value and never
clips uptake to zero.

### `config_miem_diagnostic_sectors` (character)

| | |
|---|---|
| Units | - |
| Description | Comma-separated exact MIEM sector names to publish as bounded disaggregated emissions diagnostics. Names are checked against configured sources and must remain unique after field-name sanitization. |
| Possible Values | Empty or comma-separated sector names *(default: '')* |

### `config_miem_diagnostic_categories` (character)

| | |
|---|---|
| Units | - |
| Description | Comma-separated nonnegative MIEM category IDs to publish as bounded disaggregated emissions diagnostics. |
| Possible Values | Empty or comma-separated nonnegative integers *(default: '')* |

### `config_miem_layered_diagnostics` (logical)

| | |
|---|---|
| Units | - |
| Description | Publish opt-in per-layer total emissions diagnostics and per-layer forms of every requested sector/category diagnostic. Column totals are always published when MIEM is enabled. |
| Possible Values | .true. or .false. *(default: false)* |

### `config_miem_max_diagnostic_fields` (integer)

| | |
|---|---|
| Units | - |
| Description | Hard pre-allocation cap on `species * requested sector/category groups * nVertLevels` when layered group diagnostics are enabled (the level factor is one otherwise). |
| Possible Values | Any positive integer *(default: 256)* |
